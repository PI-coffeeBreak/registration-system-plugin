...
@router.post("/metadata/{activity_id}", response_model=ActivityMetadata)
def set_metadata(
    ...
    db_metadata = db.query(ActivityMetadata).filter_by(activity_id=activity_id).first()
    
    if db_metadata:
        db_metadata.is_restricted = metadata.is_restricted
        db_metadata.slots = metadata.slots
    else:
        db_metadata = ActivityMetadata(activity_id=activity_id, **metadata.dict())
        db.add(db_metadata)
...
@router.post("/register/{activity_id}", response_model=ActivityRegistration)
def register_user(
    activity_id: int = Path(...),
    db: Session = Depends(get_db),
    ...
    existing = db.query(ActivityRegistration).filter_by(user_id=user_id, activity_id=activity_id).first()
    if existing:
        raise HTTPException(status_code=400, detail="Already registered for this activity.")

    metadata = db.query(ActivityMetadata).filter_by(activity_id=activity_id).first()
    if metadata:
        total_registered = db.query(ActivityRegistration).filter_by(activity_id=activity_id).count()
...
@router.get("/metadata/{activity_id}", response_model=ActivityMetadata)
def get_metadata(
    activity_id: int = Path(...),
    db: Session = Depends(get_db)
):
...
def is_user_registered(
    ...
    user_id = user["sub"]
    registration = db.query(ActivityRegistration).filter_by(activity_id=activity_id, user_id=user_id).first()
